...
function PetData(props) {
    ...
    // 毛孩健康狀態
    const healthOptions = ["無/尚未提供", "慢性腎衰竭", "糖尿病", "下泌尿道症候群", "體重過重", "關節炎", "腸胃敏感", "皮膚敏感", "心臟疾病", "心血管疾病"];
    ...
    useEffect(() => {
        if(selectedPet) {
            ...
            let getSelectedPet = async () => {
                try {
                    let infoResult = await axios.get(`${API_URL}/pet/data/${selectedPet}`, {withCredentials: true,});
    ...
    if(petInfo.arrDay) {
        ...
        arrDays = differenceInDays(new Date(todayY, todayM, todayD), new Date(arrY, arrM, arrD));
    }
        ...
        <div className="pet-data-card">
            <div className="row text-center">
                ...
                <div className="col-lg-6">
                    ...
                    <div className="my-3">
                        {petInfo.birthday ? (<>{`${petAgeY} 歲 ${petAgeM} 個月`}<br/></>) : ""}
                        {petInfo.arrDay ? (
