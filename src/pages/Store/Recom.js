...
function Recom() {
    ...
    useEffect(() => {
        let getPetlist = async () => {
            ...
            let response = await axios.get(`${API_URL}/store/petlist`, { withCredentials: true, });
        ...
        for (let i = 0; i < data.length; i++) {
            list.push(
                <li key={i}
                    ...
                    }}><button className="recom-pet">{i}</button></li>
            );
        }
        return list;
    };
            ...
            <section className="d-flex justify-content-center">
                <div className="recomArea d-flex">
                    <div className="mainpet ">
                        ...
                        <div className="recom-petname text-center mb-3 ">{pet ? pet.name : ''}</div>
                        <div className="recom-describe text-center ">
                            今天的牠<span className='introTag'>{pet ? pet.birthday : ''}歲</span><br />
                            是隻<span>{pet ? pet.age_category : ''}</span>的<span>{pet ? pet.category : ''}</span><br />
                            有著
                            {pet ? (pet.illnessTags.map((data) => {
                                return (
                                    <span> [ {data} ] </span>
                                )
                            })) : ''} 的問題
